  ...
  return (
    <main>
      ...
      aria-labelledby="services-heading">
        ...
        <h2 className="text-3xl md:text-4xl font-semibold py-4"  id="services-heading">
  Our Services
</h2>
        ...
        <div className="grid grid-cols-1 gap-8 md:gap-4 lg:grid-cols-4 px-4 py-6 md:grid-cols-2">
        {
          HomeServices.map((item,index)=>(
            <AnimateOnScroll direction="up" delay={0.4+index*0.1} key={index}>
            <div className="py-12  md:min-h-96 lg:min-h-78 bg-white border-[1] shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300 border-gray-200 rounded-xl px-6 flex flex-col gap-4" key={index}>
          ...
          <Link href="/services" className="text-center mt-6 shadow-xl bg-white px-6 py-2 rounded-lg border-[1] border-gray-200 active:scale-95 hover:scale-105 hover:shadow-xl hover:bg-red-700 hover:text-white transition-all duration-400">
          View All Services
          </Link>
          </div>
          </AnimateOnScroll>
      ...
      {/*Exp, num of Projects */}
      <section className = "grid grid-cols-2 space-y-8 md:grid-cols-4 bg-[#1c1f26] py-16 " aria-label="Company achievements">
        {
          ExpInfo.map((item, index)=> (
            <AnimateOnScroll direction="down" delay={0.4+index*0.2} key={index}>
            <div className="flex flex-col gap-3 text-center items-center" key={index}>
              <item.icon className="text-red-700 h-16 w-16 p-4 bg-red-700/30 rounded-full"/>
              <p className="text-white font-semibold text-4xl md:text-5xl" >{item.value}</p>
      ...
      <section className="bg-white grid grid-cols-1 md:grid-cols-2 gap-10 px-4 py-16 md:py-24">
        ...
        <div className=" space-y-6">
          ...
          <p className="text-gray-500 text-lg">
            With over 15 years of experience in the construction industry, we have established ourselves as a trusted name in civil construction. Our commitment to quality, safety, and customer satisfaction sets us apart.
          </p>
          {
