    ...
    className="bg-white/70 backdrop-blur-md text-primary-foreground fixed top-0 left-0 text-white right-0 z-50 shadow-md border-b border-black/20">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-28">
          {/* Logo */}
          <Link href="/" className="flex whitespace-nowrap shrink-none items-center">
            <div className="h-24 lg:ml-6">
             <img src="/shubh-construction-logo.png" alt="Shubh Construction – Civil & Industrial Construction Company in Gujarat"
             className="h-full w-auto"></img>
            </div>
          </Link>

          {/* Desktop Navigation */}
          <div className="hidden md:flex tracking-wide text-black items-center md:whitespace-nowrap lg:space-x-8 md:space-x-6 relative" ref={navRef}>
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`relative hover:text-red-700 font-sans font-medium pb-1 transition-colors duration-200 ${pathname === link.href ? "text-red-700" : ""}`}
              >
                {link.label}
              </Link>
            ))}
          ...
          {/* Mobile Menu Button */}
          <button className="md:hidden text-black p-2" onClick={() => setIsOpen(!isOpen)} aria-label="Toggle menu">
